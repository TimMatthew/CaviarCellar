...
const schema = z.object({
  ...
  NP_SENDER_CITY_REF: z.string().optional(),
  NP_SENDER_ADDRESS_REF: z.string().optional(),
  NP_SENDER_PHONE: z.string().optional(),
  NP_SERVICE_TYPE: z.string().default("WarehouseWarehouse"),
  NP_CARGO_TYPE: z.string().default("Parcel"),
  NP_DEFAULT_WEIGHT_KG: z.coerce.number().positive().default(1),
  SCHEDULER_ENABLED: z.enum(["true", "false"]).default("false"),
  DELIVERY_POLL_CRON: z.string().default("*/10 * * * *"),
  TRACKING_BATCH_SIZE: z.coerce.number().int().positive().max(100).default(50),
  OUTBOX_CRON: z.string().default("*/1 * * * *"),
  OUTBOX_BATCH_SIZE: z.coerce.number().int().positive().max(100).default(25),
  OUTBOX_RETRY_SECONDS: z.coerce.number().int().positive().default(60),
  OUTBOX_MAX_RETRY_SECONDS: z.coerce.number().int().positive().default(86400),
...
export const config = Object.freeze({
  ...
  novaPoshta: {
    ...
    senderRef: env.NP_SENDER_REF,
    contactSenderRef: env.NP_CONTACT_SENDER_REF,
    senderCityRef: env.NP_SENDER_CITY_REF,
    senderAddressRef: env.NP_SENDER_ADDRESS_REF,
    senderPhone: env.NP_SENDER_PHONE,
    serviceType: env.NP_SERVICE_TYPE,
    cargoType: env.NP_CARGO_TYPE,
    defaultWeightKg: env.NP_DEFAULT_WEIGHT_KG,
  },
  mail: {
    host: env.SMTP_HOST,
    port: env.SMTP_PORT,
  ...
  jobs: {
    ...
    deliveryPollCron: env.DELIVERY_POLL_CRON,
    trackingBatchSize: env.TRACKING_BATCH_SIZE,
    outboxCron: env.OUTBOX_CRON,
    outboxBatchSize: env.OUTBOX_BATCH_SIZE,
    outboxRetrySeconds: env.OUTBOX_RETRY_SECONDS,
    outboxMaxRetrySeconds: env.OUTBOX_MAX_RETRY_SECONDS,
  },
});
